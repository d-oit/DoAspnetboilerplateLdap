...
namespace Abp.Zero.Ldap.Configuration
{
    ...
    public interface ILdapSettings
    {
        ...
        Task<bool> GetUseNovellLdap(int? tenantId);

        Task<string> GetLdapHost(int? tenantId);

        Task<int> GetLdapServerPort(int? tenantId);

        Task<string> GetLdapLoginDn(int? tenantId);

        Task<string> GetLdapUserSearchBase(int? tenantId);
    }
}